...
def train_epoch_test_loss(epoch, model, optimizer, train_loader, writer, args, stock2stock_matrix=None):
    """
    train epoch function
    :param epoch: number of epoch of training
    :param model: model that will be used
...
def test_epoch(epoch, model, test_loader, writer, args, stock2stock_matrix=None, prefix='Test'):
    ...
    for i, slc in tqdm(test_loader.iter_daily(), desc=prefix, total=test_loader.daily_length):
        ...
        with torch.no_grad():
            pred = model(feature, stock2stock_matrix[stock_index][:, stock_index])
            # fix the dim issue for data after 2020 12 31
            loss = loss_fn(pred, label, args)
...
def test_epoch_test_loss(epoch, model, test_loader, writer, args, stock2stock_matrix=None, prefix='Test'):
    """
    :return: loss -> mse
             scores -> ic
             rank_ic
...
def main(args):
    ...
    for times in range(args.repeat):
        pprint('create model...')
        model = NRSR(num_relation=num_relation, d_feat=args.d_feat, num_layers=args.num_layers)
        model.to(device)
        ...
        for name in ['train', 'valid', 'test']:
            ...
            pred = inference(model, eval(name + '_loader'), stock2stock_matrix)
...
def parse_args():
    ...
    parser.add_argument('--check_mode', type=bool, default=False)
    ...
    parser.add_argument('--name', type=str, default='csi300_NRSR')
    ...
    parser.add_argument('--outdir', default='./output/csi300_NRSR')
