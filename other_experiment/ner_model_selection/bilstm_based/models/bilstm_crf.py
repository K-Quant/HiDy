...
class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size, args, crf=True):
        """功能：对LSTM的模型进行训练与测试
           参数:
            vocab_size:词典大小
            out_size:标注种类
        ...
        if not crf:
            self.model = BiLSTM(vocab_size, self.emb_size,
                                self.hidden_size, out_size, args=args).to(self.device)
        ...
        for e in range(1, self.epoches+1):
            ...
            for ind in range(0, len(word_lists), B):
                ...
                losses += self.train_step(batch_sents,
                                          batch_tags, word2id, tag2id, args)
            ...
            val_loss = self.validate(
                dev_word_lists, dev_tag_lists, word2id, tag2id, args = args)
            print("Epoch {}, Val Loss:{:.4f}".format(e, val_loss))

            if val_loss < best_score:
                best_score = val_loss
                # save the model which performs better
                if crf == True:
                    with open(args.save_BILSTM_CRF_path, "wb") as f:
                        pickle.dump(self, f)
                else:
                    with open(args.save_BILSTM_path, "wb") as f:
                        pickle.dump(self, f)
                print("Model saved: epoch = {}".format(e))


    def train_step(self, batch_sents, batch_tags, word2id, tag2id, args):
        self.model.train()
        self.step += 1
        # 准备数据
        tensorized_sents, lengths = tensorized(batch_sents, word2id)
        tensorized_sents = tensorized_sents.to(args.device)
        targets, lengths = tensorized(batch_tags, tag2id)
        targets = targets.to(args.device)

        # forward
        scores = self.model(tensorized_sents, lengths, args)

        # 计算损失 更新参数
        self.optimizer.zero_grad()
        loss = self.cal_loss_func(scores, targets, tag2id).to(args.device)
        loss.backward()
        self.optimizer.step()

        return loss.item()

    def validate(self, dev_word_lists, dev_tag_lists, word2id, tag2id, args):
        self.model.eval()
        with torch.no_grad():
            val_losses = 0.
            val_step = 0
            for ind in range(0, len(dev_word_lists), self.batch_size):
                ...
                # forward
                scores = self.model(tensorized_sents, lengths, args = args)
    ...
    def test(self, word_lists, tag_lists, word2id, tag2id, args):
        """返回最佳模型在测试集上的预测结果"""
        # 准备数据
        ...
        tensorized_sents = tensorized_sents.to(args.device)
        self.best_model.to(args.device)
        self.best_model.eval()
        with torch.no_grad():
            batch_tagids = self.best_model.test(
                tensorized_sents, lengths, tag2id, args)
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, out_size, args):
        ...
        self.bilstm = BiLSTM(vocab_size, emb_size, hidden_size, out_size, args=args)
    ...
    def forward(self, sents_tensor, lengths, args):
        # [B, L, out_size]
        emission = self.bilstm(sents_tensor, lengths, args=args)
    ...
    def test(self, test_sents_tensor, lengths, tag2id, args):
        """使用维特比算法进行解码"""
        ...
        crf_scores = self.forward(test_sents_tensor, lengths, args)
        # device = crf_scores.device
        # B:batch_size, L:max_len, T:target set size
        B, L, T, _ = crf_scores.size()
        # viterbi[i, j, k]表示第i个句子，第j个字对应第k个标记的最大分数
        viterbi = torch.zeros(B, L, T).to(args.device)
        # backpointer[i, j, k]表示第i个句子，第j个字对应第k个标记时前一个标记的id，用于回溯
        backpointer = (torch.zeros(B, L, T).long() * end_id).to(args.device)
        ...
        for step in range(L-1, 0, -1):
            ...
            if step == L-1:
                ...
                new_in_batch = torch.LongTensor(
                    [end_id] * (batch_size_t - prev_batch_size_t)).to(args.device)
                offset = torch.cat(
                    [tags_t, new_in_batch],
                    dim=0
                )  # 这个offset实际上就是前一时刻的
